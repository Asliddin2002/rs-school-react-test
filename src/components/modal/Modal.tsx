    ...
    >
      <div className="bg-white w-[500px] lg:w-[1000px] lg:h-[700px] fixed flex flex-col sm:flex-row mt-[130px]">
        ...
        <div className="sm:w-1/2 flex items-center justify-center">
          <img className=" h-[300px] sm:h-[700px] sm:w-full" src={image} alt={`${name}`} />
        </div>
        <div className="mt-[50px] sm:w-1/2 ">
          <h1 className=" text-center font-semibold text-[28px]">{name}</h1>
          <div className="px-[20px] py-[30px]">
            ...
            <ul className="mt-[20px] flex flex-col gap-2">
              <li>Academy Award Nominations - {academyAwardNominations}</li>
              <li>Academy Award Wins - {academyAwardWins}</li>
              <li>Box Off Ice Revenue in Millions - {boxOfficeRevenueInMillions}</li>
              <li>Budget in Millions - {budgetInMillions}</li>
              <li>Rotten Tomatoes Score - {rottenTomatoesScore}</li>
              <li>Run time in minutes - {runtimeInMinutes}</li>
            </ul>
          </div>
        </div>
      </div>
